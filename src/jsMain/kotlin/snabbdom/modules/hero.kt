...
class HeroModule : Module() {
    ...
    override val pre get() = {
        removed = newObj()
        created = mutableListOf<Any?>()
    }

    override val create get() = { oldVnode: VNode, vnode: VNode ->
    ...
    override val destroy get() = { vnode: VNode ->
        val hero = vnode.data?.hero
        if (hero?.id != null) {
            val elm = vnode.elm
